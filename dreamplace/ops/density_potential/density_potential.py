...
try:
    import dreamplace.ops.density_potential.density_potential_hip as density_potential_hip
except:
    pass
...
class DensityPotentialFunction(Function):
    ...
    def forward(
        ...
        if pos.is_cuda:
            output = density_potential_hip.forward(
                    pos.view(pos.numel()),
                    node_size_x, node_size_y,
                    ax, bx, cx,
                    ay, by, cy,
    ...
    def backward(ctx, grad_pos):
        if grad_pos.is_cuda:
            output = density_potential_hip.backward(
                    grad_pos,
...
class DensityPotential(nn.Module):
    ...
    def forward(self, pos):
        if self.initial_density_map is None:
            ...
            if pos.is_cuda:
                self.initial_density_map = density_potential_hip.fixed_density_map(
                        pos.view(pos.numel()),
