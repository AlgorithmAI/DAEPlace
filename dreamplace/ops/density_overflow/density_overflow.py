...
try:
    import dreamplace.ops.density_overflow.density_overflow_hip_thread_map as density_overflow_hip_thread_map
    import dreamplace.ops.density_overflow.density_overflow_hip_by_node as density_overflow_hip_by_node
except:
    pass
    ...
    def forward(
        ...
        if pos.is_cuda:
            if algorithm == 'threadmap':
                output = density_overflow_hip_thread_map.forward(
                        pos.view(pos.numel()),
                        node_size_x,
                        node_size_y,
                        bin_center_x,
                ...
                output = density_overflow_hip_by_node.forward(
                        pos.view(pos.numel()),
                        node_size_x,
                        node_size_y,
                        bin_center_x,
...
class DensityOverflow(object):
    ...
    def __init__(self, node_size_x, node_size_y, bin_center_x, bin_center_y, target_density, xl, yl, xh, yh, bin_size_x, bin_size_y, num_movable_nodes, num_terminals, num_filler_nodes, algorithm='by-node', num_threads=8):
        ...
        if algorithm == 'threadmap' and node_size_x.is_cuda:
            ...
            density_overflow_hip_thread_map.thread_map(
    ...
    def forward(self, pos):
        ...
        if self.initial_density_map is None:
            ...
            if pos.is_cuda:
                self.initial_density_map = density_overflow_hip_thread_map.fixed_density_map(
                        pos.view(pos.numel()),
                        self.node_size_x,
                        self.node_size_y,
                        self.bin_center_x,
