...
class PlaceDrawer (object):
    ...
    def forward(
        ...
        try:
            ...
            def normalize_x(xx):
                return (xx - (layout_xl-padding*bin_size_x))/(layout_xh-layout_xl+padding*2*bin_size_x)*width
            def normalize_y(xx):
                return (xx - (layout_yl-padding*bin_size_y))/(layout_yh-layout_yl+padding*2*bin_size_y)*height
            ...
            if iteration:
                ...
                ctx.move_to(normalize_x((xl+xh)/2), normalize_y((yl+yh)/2))
                ctx.show_text('{:04}'.format(iteration))

            surface.write_to_png(filename)  # Output to PNG
            print("[I] plotting to %s takes %.3f seconds" % (filename, time.time()-tt))
