...
class PlaceDrawer
{
    public:
        ...
        /// constructor
        PlaceDrawer(const coordinate_type* x, const coordinate_type* y,
    ...
    protected:
        ...
        void paintCairo(cairo_surface_t* cs, double width, double height) const
        {
            ...
            {
                // fixed macro
                for (int i = m_num_movable_nodes; i < m_num_nodes-m_num_filler_nodes; ++i)
                {
        ...
        void getPinBbox(index_type i, double scale_rato, coordinate_type& xl, coordinate_type& yl, coordinate_type& xh, coordinate_type& yh) const
        {
            ...
            coordinate_type pin_size = std::max(std::min(m_site_width, m_row_height)/10, (coordinate_type)(1.0/scale_rato));
