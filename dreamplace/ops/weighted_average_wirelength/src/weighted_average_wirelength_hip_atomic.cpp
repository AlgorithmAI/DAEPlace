 /**
 * @file   weighted_average_wirelength_hip_atomic.cpp
 * @author Xu Li
 * @date   10 2024
 */
        ...
        at::Tensor gamma)
{
    ...
    at::Tensor xy_max = at::full({2, num_nets}, std::numeric_limits<V>::min(), pos.options().dtype(at::kInt));
    at::Tensor xy_min = at::full({2, num_nets}, std::numeric_limits<V>::max(), pos.options().dtype(at::kInt));

    AT_DISPATCH_FLOATING_TYPES(pos.type(), "computeWeightedAverageWirelengthHipAtomicLauncher", [&] {
            computeWeightedAverageWirelengthHipAtomicLauncher<scalar_t, V>(
                    pos.data_ptr<scalar_t>(), pos.data_ptr<scalar_t>()+num_pins,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("forward", &DREAMPLACE_NAMESPACE::weighted_average_wirelength_atomic_forward, "WeightedAverageWirelength forward (HIP)");
  m.def("backward", &DREAMPLACE_NAMESPACE::weighted_average_wirelength_atomic_backward, "WeightedAverageWirelength backward (HIP)");
}