...
class ElectricPotentialOpTest(unittest.TestCase):
    def test_densityOverflowRandom(self):
        ...
        if torch.cuda.device_count(): 
            custom_hip = electric_potential.ElectricPotential(
            ...
            #pos.grad.zero_()
            result_hip = custom_hip.forward(pos)
            print("custom_result_hip = ", result_hip.data.cpu())
            print(result_hip.type())
            result_hip.backward()
            grad_hip = pos.grad.clone()
            print("custom_grad_hip = ", grad_hip.data.cpu())

            #np.testing.assert_allclose(result.detach().numpy(), result_hip.data.cpu().detach().numpy())
...
def eval_runtime(design):
    ...
    tt = time.time()
    for i in range(iters): 
        result = custom_hip.forward(pos_var)
        result.backward()
    torch.cuda.synchronize()
    print("custom_hip takes %.3f ms" % ((time.time()-tt)/iters*1000))
