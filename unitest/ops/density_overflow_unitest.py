...
class DensityOverflowOpTest(unittest.TestCase):
    def test_densityOverflowRandom(self):
        ...
        if torch.cuda.device_count(): 
            custom_hip = density_overflow.DensityOverflow(
            ...
            result_hip, max_density_hip = custom_hip.forward(pos)
            print("by-node custom_result = ", result_hip.data.cpu())
            print("by-node custom_max_density_hip = ", max_density_hip.data.cpu())

            np.testing.assert_allclose(result, result_hip.data.cpu())
            np.testing.assert_allclose(max_density, max_density_hip.data.cpu())
        ...
        if torch.cuda.device_count(): 
            ...
            pos = Variable(torch.from_numpy(np.concatenate([xx, yy]))).cuda()
            result_hip, max_density_hip = custom_hip.forward(pos)
            print("threadmap custom_result = ", result_hip.data.cpu())
            print("threadmap custom_max_density_cuda = ", max_density_hip.data.cpu())
...
def eval_runtime(design):
    ...
    custom_hip_by_node = density_overflow.DensityOverflow(
                torch.from_numpy(node_size_x).cuda(), torch.from_numpy(node_size_y).cuda(), 
                torch.from_numpy(bin_center_x).cuda(), torch.from_numpy(bin_center_y).cuda(), 
                target_density=target_density, 
                xl=xl, yl=yl, xh=xh, yh=yh, 
    ...
    custom_hip_thread_map = density_overflow.DensityOverflow(
                torch.from_numpy(node_size_x).cuda(), torch.from_numpy(node_size_y).cuda(), 
                torch.from_numpy(bin_center_x).cuda(), torch.from_numpy(bin_center_y).cuda(), 
                target_density=target_density, 
                xl=xl, yl=yl, xh=xh, yh=yh, 
    ...
    for i in range(iters): 
        result = custom_hip_by_node.forward(pos_var)
    torch.cuda.synchronize()
    print("custom_cuda_by_node takes %.3f ms" % ((time.time()-tt)/iters*1000))

    tt = time.time()
    for i in range(iters): 
        result = custom_hip_thread_map.forward(pos_var)
    torch.cuda.synchronize()
