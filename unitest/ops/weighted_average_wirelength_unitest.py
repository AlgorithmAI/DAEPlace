...
class WeightedAverageWirelengthOpTest(unittest.TestCase):
    def test_weighted_average_wirelength_random(self):
        ...
        if torch.cuda.device_count(): 
            ...
            custom_hip = weighted_average_wirelength.WeightedAverageWirelength(
            ...
            np.testing.assert_allclose(result_hip.data.cpu().numpy(), golden_value, atol=1e-6)
            np.testing.assert_allclose(grad_hip.data.cpu().numpy(), grad.data.numpy(), rtol=1e-6, atol=1e-7)
        ...
        if torch.cuda.device_count(): 
            ...
            result_hip = custom_hip.forward(pin_pos_var.cuda())
            print("custom_hip_result sparse = ", result_hip.data.cpu())
            result_hip.backward()
            grad_hip = pin_pos_var.grad.clone()
            print("custom_grad_hip sparse = ", grad_hip.data.cpu())
